...
class DashboardApp {
    constructor() {
        this.config = {
            apiBaseUrl: '/ccc/api.php',
            authBaseUrl: '/ccc/auth.php',
    ...
    async makeApiRequest(endpoint) {
        const dateParam = this.displayMode === 'tomorrow' ? '&date=tomorrow' : '';
        const url = `${this.config.apiBaseUrl}?endpoint=${endpoint}${dateParam}&v=${Date.now()}`;
        ...
        try {
            ...
            if (!contentType?.includes('application/json')) {
                ...
                throw new Error('Server returned non-JSON response. Check your PHP configuration.');
            }
    ...
    renderAddAccountPrompt() {
        ...
        calendarContent.innerHTML = `
            <div style="text-align: center; padding: 30px; background: white; border-radius: 15px; margin: 15px 0;">
                ...
                <button onclick="window.location.href='add-account.html'" class="auth-button">
                    ➕ Add Google Account
                </button>
